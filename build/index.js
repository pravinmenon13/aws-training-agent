...
// ── resource data ──────────────────────────────────────────────────────────
import { TOURNAMENTS, CERTIFICATIONS, EXTERNAL_LEARNING_PATHS, CPE_MODULES, GENAI_PATHS, RESKILLING_TRACKS, ACCOUNT_LEARNING_PATHS, WELL_ARCHITECTED_SESSIONS, TECH_TALKS, IMMERSION_DAYS, DISCOVERY_SERIES, HACKATHON_RECORDINGS, IBM_BLOGS, FAQS, } from "./resources.js";
// ── enrolment / reminder store ─────────────────────────────────────────────
import { upsertEnrolment, getEnrolment, listEnrolments, deleteEnrolment, markReminded, hasBeenReminded, listReminders, } from "./store.js";
...
// ════════════════════════════════════════════════════════════════════════════
server.tool("query_events", "Search and filter AWS training calendar events. Supports filtering by category, keyword in title, and/or a date range.", {
    ...
    if (keyword)
        results = results.filter(e => matchesKeyword(e.title, keyword));
    if (from_date)
        results = results.filter(e => e.startDate >= from_date);
    if (to_date)
        results = results.filter(e => e.startDate <= to_date);
    if (!results.length)
        return { content: [{ type: "text", text: "No events matched your query." }] };
...
// ════════════════════════════════════════════════════════════════════════════
server.tool("manage_enrolment", "Enrol a user in a training event, update their enrolment status, list their enrolments, or cancel an enrolment.", {
    ...
    switch (action) {
        case "enrol": {
            ...
            if (!event)
                return { content: [{ type: "text", text: `No event with ID ${event_id}.` }], isError: true };
            const existing = getEnrolment(user_name, event_id);
            if (existing)
                return { content: [{ type: "text", text: `${user_name} is already enrolled (status: ${existing.status}).` }] };
        ...
        case "update_status": {
            ...
            const existing = getEnrolment(user_name, event_id);
            if (!existing)
                return { content: [{ type: "text", text: `No enrolment found for ${user_name} in event ${event_id}.` }], isError: true };
            const updated = upsertEnrolment({ ...existing, status: status, notes: notes ?? existing.notes });
        ...
        case "list": {
            ...
            if (!records.length)
                return { content: [{ type: "text", text: `No enrolments found for ${user_name}.` }] };
...
// ════════════════════════════════════════════════════════════════════════════
server.tool("check_reminders", "List upcoming training events within N days, mark an event as reminded, or list reminded events.", {
    ...
    switch (action) {
        case "list_upcoming": {
            ...
            if (!upcoming.length)
                return { content: [{ type: "text", text: `No events in the next ${horizon} day(s).` }] };
        ...
        case "mark_reminded": {
            if (!event_id || !user_name)
                return { content: [{ type: "text", text: "event_id and user_name are required." }], isError: true };
        ...
        case "list_reminded": {
            ...
            if (!records.length)
                return { content: [{ type: "text", text: `No reminder records found.` }] };
...
// ════════════════════════════════════════════════════════════════════════════
server.tool("query_certifications", "Look up AWS certification learning paths by level or keyword. Returns IBM YourLearning links and official AWS certification links.", {
    ...
    if (keyword)
        results = results.filter(c => matchesKeyword(c.name, keyword));
    if (!results.length)
        return { content: [{ type: "text", text: "No certifications matched." }] };
...
// ════════════════════════════════════════════════════════════════════════════
server.tool("query_resources", "Search across all AWS resource categories: tech_talks, immersion_days, discovery_series, hackathon_recordings, cpe_modules, genai_paths, reskilling_tracks, external_learning_paths, account_learning_paths, well_architected, blogs, tournaments.", {
    category: z.enum([
    ...
    const sections = [];
    const include = (cat) => category === "all" || category === cat;
    ...
    if (!output.trim())
        return { content: [{ type: "text", text: "No resources matched." }] };
    return { content: [{ type: "text", text: output }] };
});
// ════════════════════════════════════════════════════════════════════════════
//  TOOL 6 — recommend_learning_path
// ════════════════════════════════════════════════════════════════════════════
server.tool("recommend_learning_path", "Recommend an AWS learning path based on a user's role, experience level, and goal.", {
    ...
    const lines = [`Recommended AWS learning path for: ${role} (${level})${goal ? ` — focus: ${goal}` : ""}\n`];
...
//  TOOL 7 — query_faq
// ════════════════════════════════════════════════════════════════════════════
server.tool("query_faq", "Search the IBM AWS certification FAQ. Find answers about LRT, vouchers, CAP, Credly, exam scheduling, Skill Builder, HCAM, and more. Supports keyword search or tag filtering.", {
    keyword: z.string().optional()
    ...
    if (id !== undefined) {
        ...
        if (!entry)
            return { content: [{ type: "text", text: `No FAQ with ID ${id}.` }] };
    ...
    if (keyword) {
        const kw = keyword.toLowerCase();
        results = results.filter(f => f.question.toLowerCase().includes(kw) ||
            f.answer.toLowerCase().includes(kw) ||
            f.tags.some(t => t.toLowerCase().includes(kw)));
    }
    if (!results.length)
        return { content: [{ type: "text", text: "No FAQ entries matched your query." }] };
    const lines = results.map(f => `Q${f.id}: ${f.question}\n  A: ${f.answer}\n  Tags: ${f.tags.join(", ")}`);
